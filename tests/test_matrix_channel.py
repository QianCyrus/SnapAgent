...
import snapagent.channels.matrix as matrix_module
from snapagent.bus.events import OutboundMessage
from snapagent.bus.queue import MessageBus
from snapagent.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from snapagent.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr("snapagent.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "snapagent.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("snapagent.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr("snapagent.channels.matrix.asyncio.create_task", _fake_create_task)
