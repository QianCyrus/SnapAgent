...
from snapagent.config.schema import TelegramConfig
...
class TelegramChannel(BaseChannel):
    ...
    def __init__(
        ...
        self._progress_messages: dict[str, int] = {}  # chat_id -> progress message_id
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        # Tool hint progress → edit a single status message in-place (keep typing)
        if is_progress and is_tool_hint:
            await self._update_progress(msg)
            return

        # Text streaming progress → send normally but keep typing
        if is_progress:
            pass  # fall through to normal send without stopping typing
        else:
            # Final message → clean up progress indicator and stop typing
            await self._clear_progress(msg.chat_id)
    ...
    async def _update_progress(self, msg: OutboundMessage) -> None:
        """Create or edit a progress status message in-place."""
        try:
            chat_id = int(msg.chat_id)
        except ValueError:
            return
        ...
        existing_msg_id = self._progress_messages.get(msg.chat_id)
        if existing_msg_id:
            try:
                ...
                self._progress_messages.pop(msg.chat_id, None)

        try:
            sent = await self._app.bot.send_message(chat_id=chat_id, text=text)
            self._progress_messages[msg.chat_id] = sent.message_id
        except Exception as e:
            logger.debug("Failed to send progress message: {}", e)

    async def _clear_progress(self, chat_id: str) -> None:
        """Delete the progress status message for a chat."""
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm snapagent.\n\n"
    ...
    async def _on_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if media_file and self._app:
            try:
                ...
                media_dir = Path.home() / ".snapagent" / "media"
                ...
                if media_type == "voice" or media_type == "audio":
                    from snapagent.providers.transcription import GroqTranscriptionProvider
